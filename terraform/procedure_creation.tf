...
try {
    ...
    if (failedTables.length > 0) {
        ...
        // Send an alert using the notification integration
        var state2 = snowflake.createStatement({
            sqlText: `CALL SYSTEM$SEND_EMAIL('dynamic_table_refresh_notification_integration', '${var.dev_qa_alerts_email}', 'Dev Dynamic Table Refresh Failures', :1);`,
            binds: [emailContent]
        });
        var alertResult = state2.execute();
...
try {
    ...
    if (failedTables.length > 0) {
        ...
        // Send an alert using the notification integration
        var state2 = snowflake.createStatement({
            sqlText: `CALL SYSTEM$SEND_EMAIL('dynamic_table_refresh_notification_integration', '${var.dev_qa_alerts_email}', 'QA Dynamic Table Refresh Failures', :1);`,
            binds: [emailContent]
        });
        var alertResult = state2.execute();
...
try {
    ...
    if (failedTables.length > 0) {
        ...
        // Send an alert using the notification integration
        var state2 = snowflake.createStatement({
            sqlText: `CALL SYSTEM$SEND_EMAIL('dynamic_table_refresh_notification_integration', '${var.alerts_email}', 'Dynamic Table Refresh Failures', :1);`,
            binds: [emailContent]
        });
        var alertResult = state2.execute();
...
resource "snowflake_procedure" "check_raintree_acknowledge_file" {
  name     = "CHECK_RAINTREE_ACKNOWLEDGE_FILE"
  database = var.landing
  ...
  statement           = <<EOF
  try {
  // Step 1: Set the default schema for the session
  var setSchemaQuery = `USE SCHEMA LANDING.RAINTREE`;
  snowflake.execute({ sqlText: setSchemaQuery });
  ...
  var checkLoadingMessageTable = `SELECT 
                                    CASE 
                                        WHEN DATE(MAX(MESSAGE_DATETIME)) < DATE(CONVERT_TIMEZONE(''America/Los_Angeles'', ''America/New_York'', CURRENT_TIMESTAMP)) THEN 'YES'
                                        ELSE 'NO'
                                    END AS shouldCheck
  ...
  if (excuteCheckLoadingMessageTable.next())  {
      var shouldCheck = excuteCheckLoadingMessageTable.getColumnValue(1);
          ...
          while (fileListResultSet.next()) {
              ...
              if (!isNaN(folderName)) {
                  batchFolders.push(parseInt(folderName));
              }
          }
          ...
          // Step 5: Check for _SUCCESS file in the latest batch folder
              if (largestAuditBatch < largestStageBatch) {
                  var largestStageBatchFolderPath = `@snowflake_raintree_stage/i00255/incremental/$${largestStageBatch}/`;
                  var checkSuccessFileQuery = `LIST $${largestStageBatchFolderPath}`;
                  var fileResultSet = snowflake.execute({ sqlText: checkSuccessFileQuery });
                  ...
                  var insertRaintreeLoadTrackingTableQuery = `
                              ...
                              VALUES ((CAST(CONVERT_TIMEZONE('America/Los_Angeles', 'America/New_York', CURRENT_TIMESTAMP) AS TIMESTAMP_NTZ)), 'NEW')`;
                      try {
